...
```{r}
x %>% 
  summarise(cc = cor(faculty, earnings),
            sat_earn = cor(sat, earnings)) 
...
```{r}
facultyearnings <- lm(earnings ~ faculty, data = x)

ggplot(x, aes(x = faculty, y = earnings)) + geom_point() +
  geom_smooth(method = "lm", se = F)

ggplot(x, aes(x = sat, y = earnings)) + geom_point() +
  geom_smooth(method = "lm", se = F)
```
...
```{r}
multiple <- lm(earnings ~ faculty + sat, data = x)
summary(multiple) %>% 
  tidy()

ggplot(x, aes(x = earnings, y = faculty, color = sat)) + geom_point()
```
...
treatment: cutting the financial aid leads to a 100 point downward trend in the avergae sat score
outcome: median earnings 10 years after entering the workforce
casual effect: the difference in median earnings associated with the 100 point decline in mean SAT scores

the casual effect on median earnings is the difference between what median earnings would have been under the old policy and what the median earnings would have been under the new policy
problem: You cannot observe the causual effet on a single unit. 
Assumption: No confounding variables, assignment of treatment is entirely random, 
